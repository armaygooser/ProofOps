...
ProofOps 以 CyberGuard v0.14（本地基线提交 `5893bd3`）的治理内核为基座，来源是：
...
| 独立 recovery verifier | 温度、功率与心跳三项执行后独立观测 |
...
- 哈希、HMAC、状态门、审批绑定与回滚均由 FastAPI 后端真实计算。
- 设备状态来自确定性数字孪生，适合比赛录屏，不代表真实现场接入。
- Agent 调查轨迹是确定性编排。部署并保存 AgentTeams Worker 回执后，才可标注为 live run。
